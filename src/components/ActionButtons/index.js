...
export default function ActionButtons({ onSubmit }) {
  ...
  const defineRoute = (e) => {
    e.preventDefault();
    if (submitButtonCondition) return onSubmit();
    else if (pathname === "/perfil") return history.push("/perfil/editar");
    else if (pathname === "/produtos") return history.push("/produtos/novo");
  };
